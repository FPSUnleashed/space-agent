...
- current first-party surface-tag examples are framework-owned runtime tags `runtime-browser` or `runtime-app`, `onscreen` from `_core/onscreen_agent`, `admin` from `_core/admin`, and feature-owned tags such as `route:<current-path>` or `space:open`
