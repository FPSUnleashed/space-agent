...
- packaged desktop runs may activate `data-space-inject` through the Electron frame-preload hook, but that hook must accept only same-origin script URLs whose normalized path starts with `/mod/` and must reject remote origins, query or hash decorations, and path-trick variants
